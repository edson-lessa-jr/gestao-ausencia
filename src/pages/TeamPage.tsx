...
export default function TeamPage({ user }: { user: User }) {
  ...
  const openCreate = () => { setEditingId(null); setForm(initial); setOpen(true) }
  ...
  return <>
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={2} mb={3}><Box><Typography variant="h4">Equipe</Typography><Typography color="text.secondary">Colaboradores, vínculos e regras individuais de férias.</Typography></Box>{user.role === 'ADMIN' && <Stack direction="row" spacing={1}><Button variant="outlined" onClick={() => setTeamOpen(true)}>Nova equipe</Button><Button variant="contained" startIcon={<AddRounded />} onClick={openCreate}>Novo colaborador</Button></Stack>}</Stack>
    ...
    <Dialog open={open} onClose={() => setOpen(false)} fullWidth maxWidth="md"><DialogTitle>{editingId ? 'Editar colaborador' : 'Novo colaborador'}</DialogTitle><DialogContent><Grid container spacing={2} mt={.5}>
      ...
      <Grid size={{ xs: 12, sm: 3 }}><FormControl fullWidth><InputLabel>Equipe</InputLabel><Select label="Equipe" value={form.team_id || ''} onChange={(e) => setForm({ ...form, team_id: e.target.value })}>{teams.map((team) => <MenuItem key={team.id} value={team.id}>{team.name}</MenuItem>)}</Select></FormControl></Grid>
