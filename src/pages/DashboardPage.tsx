...
interface Dashboard { currentBalance: number; projectedYearEnd: number; pendingCount: number; administrativeBalance: number; projectedAdministrativeBalance: number; projection: Array<{ month: string; confirmed: number; projected: number }>; quotas: { justified: { used: number; available: number; limit: number }; unjustified: { used: number; available: number; limit: number } } }
...
export default function DashboardPage({ user }: { user: User }) {
  ...
  const upcoming = requests.filter((r) => r.end_date >= new Date().toISOString().slice(0, 10) && ['REQUESTED', 'APPROVED', 'QUANTUM_REGISTERED', 'QUANTUM_APPROVED'].includes(r.status)).slice(0, 5)
  return <>
    ...
    <Grid container spacing={2} mb={2.5}>
      <Grid size={{ xs: 12, sm: 6, lg: 3 }}><Summary icon={<BeachAccessOutlined />} label="Saldo oficial de férias" value={`${data.currentBalance.toLocaleString('pt-BR')} dias`} accent="#0799b3" /></Grid>
      <Grid size={{ xs: 12, sm: 6, lg: 3 }}><Summary icon={<EventAvailableOutlined />} label="Férias projetadas em dezembro" value={`${data.projectedYearEnd.toLocaleString('pt-BR')} dias`} accent="#075e86" /></Grid>
      <Grid size={{ xs: 12, sm: 6, lg: 3 }}><Summary icon={<EventAvailableOutlined />} label="Saldo administrativo" value={`${data.administrativeBalance.toLocaleString('pt-BR')} dias`} accent="#5a3aa6" /></Grid>
      <Grid size={{ xs: 12, sm: 6, lg: 3 }}><Summary icon={<HourglassTopOutlined />} label="Ausências pendentes" value={`${data.pendingCount} ${data.pendingCount === 1 ? 'solicitação' : 'solicitações'}`} accent="#e4680b" /></Grid>
    </Grid>
    <Grid container spacing={2.5}>
      <Grid size={{ xs: 12, lg: 8 }}><Paper variant="outlined" sx={{ p: { xs: 2, sm: 2.5 }, height: 410 }}><Typography variant="h6">Projeção de saldo de férias</Typography><Typography variant="body2" color="text.secondary" mb={2}>O saldo oficial desconta apenas ausências aprovadas no Quantum; a projeção inclui os status anteriores.</Typography><ResponsiveContainer width="100%" height={310}><AreaChart data={data.projection} margin={{ left: -20, right: 8 }}><defs><linearGradient id="confirmed" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#45c0cf" stopOpacity={.34} /><stop offset="100%" stopColor="#45c0cf" stopOpacity={.02} /></linearGradient></defs><CartesianGrid vertical={false} stroke="#e8eef2" /><XAxis dataKey="month" tickLine={false} axisLine={false} /><YAxis tickLine={false} axisLine={false} /><Tooltip formatter={(value) => `${Number(value).toLocaleString('pt-BR')} dias`} /><Legend /><Area name="Saldo oficial" type="monotone" dataKey="confirmed" stroke="#075e86" fill="url(#confirmed)" strokeWidth={3} /><Area name="Saldo projetado" type="monotone" dataKey="projected" stroke="#ff8d33" fill="transparent" strokeWidth={2} strokeDasharray="6 5" /></AreaChart></ResponsiveContainer></Paper></Grid>
      <Grid size={{ xs: 12, lg: 4 }}><Paper variant="outlined" sx={{ p: 2.5, height: 410, overflow: 'auto' }}><Typography variant="h6" mb={2}>Próximas ausências</Typography><Stack spacing={2}>{upcoming.length ? upcoming.map((r) => <Box key={r.id} sx={{ display: 'flex', gap: 1.5, alignItems: 'center' }}><Box sx={{ width: 8, height: 42, borderRadius: 4, bgcolor: r.status === 'QUANTUM_APPROVED' ? 'secondary.main' : 'warning.main' }} /><Box sx={{ minWidth: 0, flex: 1 }}><Typography variant="body2" fontWeight={700} noWrap>{r.user_name}</Typography><Typography variant="caption" color="text.secondary">{absenceLabels[r.type]} · {formatDate(r.start_date)}</Typography></Box><StatusChip status={r.status} /></Box>) : <Typography color="text.secondary">Nenhuma ausência futura.</Typography>}</Stack></Paper></Grid>
