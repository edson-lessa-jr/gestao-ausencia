...
const emptyForm = { date: '', name: '', team_id: '' }
...
export default function HolidaysPage({ user }: { user: User }) {
  ...
  const openEdit = (holiday: Holiday) => { setEditingId(holiday.id); setForm({ date: holiday.date, name: holiday.name, team_id: holiday.team_id || '' }); setOpen(true) }
  ...
  const importCsv = async (file: File) => {
    ...
    try {
      ...
      const dateIndex = headers.findIndex((header) => ['data', 'date'].includes(header)); const nameIndex = headers.findIndex((header) => ['nome', 'name', 'descricao', 'feriado'].includes(header)); const teamIndex = headers.findIndex((header) => ['equipe', 'team', 'team_id'].includes(header))
      ...
      const holidays = lines.slice(1).map((line, index) => {
        ...
        return { date: csvDate(columns[dateIndex]), name: columns[nameIndex], team_id: user.role === 'SUPERVISOR' ? user.team_id : team?.id || null }
      })
  ...
  return <>
    ...
    <Alert severity="info" sx={{ mb: 2 }}>CSV aceito: <strong>data;nome;equipe</strong>. Use datas em DD/MM/AAAA ou AAAA-MM-DD. A equipe é opcional para administradores.</Alert>
