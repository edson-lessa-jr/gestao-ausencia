...
export default function RequestsPage({ user }: { user: User }) {
  ...
  return <>
    <Stack direction={{ xs: 'column', sm: 'row' }} justifyContent="space-between" alignItems={{ sm: 'center' }} spacing={2} mb={3}><Box><Typography variant="h4">Solicitações</Typography><Typography color="text.secondary">Novas solicitações aguardam análise do supervisor; o solicitante pode cancelar enquanto estiverem pendentes.</Typography></Box><Button variant="contained" startIcon={<AddRounded />} onClick={() => setDrawer(true)}>Nova solicitação</Button></Stack>
